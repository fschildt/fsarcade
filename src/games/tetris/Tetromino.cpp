...
bool Tetromino::Update(uint16_t *board_bitmap, int32_t drotation, int32_t dx, int32_t dy) {
    uint8_t target_orientation = (m_Orientation + drotation) % 4;
    if (CollidesWithBoard(board_bitmap, m_Id, target_orientation, m_X, m_Y)) {
        m_Orientation = target_orientation;
    }

    int8_t target_x = m_X + dx;
    if (CollidesWithBoard(board_bitmap, m_Id, m_Orientation, target_x, m_Y)) {
        m_X = target_x;
    }
